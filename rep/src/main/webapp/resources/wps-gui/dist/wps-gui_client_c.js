...
wps.hiddenForm = function(unmarshaller, options, url, fields, body) {
  ...
  $('body').append('<form classs="x-hidden" action="'+url+'" method="POST" target="iframe" encType="multipart/form-data" accept-charset="utf-8" id="hiddenform-form"></form>');
...
//	  console.log(this.responseText);
	  ...
	  saveByteArray(this.response, 'result.tif');
//	  var bb = new window.WebKitBlobBuilder();
//      bb.append(this.response); // Note: not xhr.responseText
//
//      var blob = bb.getBlob('tiff');
...
wps.process.prototype.setInputData = function(input, data) {
  ...
  if (data instanceof wps.process.chainlink) {
        ...
        value: {
          domainSubset: {
            boundingBox: {
              ...
              value: {
                crs: 'http://www.opengis.net/gml/srs/epsg.xml#3857',
    ...
    this.info.value.dataInputs.input.push(inputValue);
  } else if (data instanceof wps.process.remoteWCS) {
	    inputValue = {
	      identifier: {
	        value: input.identifier.value
	      },
    ...
    this.info.value.dataInputs.input.push(inputValue);
  } else if (data instanceof wps.process.remoteWFS) {
	    var reqUrl = data.url + "?" + "service=WFS&version=1.1.0&request=GetFeature&typeName=" + data.typeName;
	    if (data.srsName != "") {
	    	reqUrl = reqUrl + "&srsName=" + data.srsName;
	    }
	    inputValue = {
	      identifier: {
	        value: input.identifier.value
...
wps.process.remoteWCS = function(options) {
  this.lowerCorner = null;
  this.upperCorner = null;
  this.identifier = null;
  this.url= null;
...
wps.process.remoteWFS = function(options) {
	  this.srsName = null;
	  this.typeName = null;
	  this.bbox = null;
	  this.url= null;
