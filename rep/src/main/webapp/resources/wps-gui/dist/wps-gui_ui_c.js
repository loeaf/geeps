...
wps.backgroundLayer = new ol.layer.Tile({
  ...
  source: new ol.source.TileWMS({
    ...
    params: {
      'TILED': 'true',
      'VERSION': mapWmsServerVersion,
      'LAYERS': 'korea_sgg',
      'FORMAT': mapWmsServerFormat
    }
  })
});
...
wps.mapSettings = {
  ...
  //center: [228089, 297167],
  center: [14139000, 4500000],
  //extent: extent,
  zoom: 9,
  minZoom: 1
};
...
wps.editor.prototype.showEditForm = function(node) {
  ...
  if (node._info.literalData) {
    ...
    if (rasterLayer === true) {
      ...
      if (wcsServers.length > 0) {
    	  for (var i = 0; i < wcsServers.length; i++) {
    		  html += '<option value="" selected disabled>' + wcsServers[i].alias + '</option>';
    		  var cs = wcsServers[i].coverages;
    		  var csLength = cs.length;
    		  for (var j = 0; j < csLength; j++) {
    			  html += '<option value="' + wcsServers[i].serverId + "_" + prefix + cs[j].name + '">' + cs[j].name + "</option>";
			}
		}
       }
      html += "</select>";
      ...
      //add
      if (wfsServers.length > 0) {
    	  for (var i = 0; i < wfsServers.length; i++) {
    		  html += '<option value="" selected disabled>' + wfsServers[i].alias + '</option>';
    		  var fts = wfsServers[i].featureTypes;
    		  var ftsLength = fts.length;
    		  for (var j = 0; j < ftsLength; j++) {
    			  // selected 구현해야함.
    			  html += '<option value="' + wfsServers[i].serverId + "_" + prefix + fts[j].name + '">' + fts[j].name + "</option>";
			}
		}
    ...
    html += '<div class="form-row"><label for="' + id + '">' + nameText + '</label>';
    html += '</div>';
    html += '<p class="form-row"><small>Provide bounding box in <b>EPSG:3857</b></small></p>';
    html += '<div class="form-row" id="' + name + '-field">';
...
wps.ui.prototype.handleLocal = function(value) {
  ...
  if (typeof value === "string" && value.indexOf(wps.VECTORLAYER) !== -1) {
    ...
    if (wfsServers.length>0) {
    	for (var i = 0; i < wfsServers.length; i++) {
    		if (value.indexOf(wfsServers[i].serverId)>-1) {
    			return new wps.process.remoteWFS({
			      srsName: 'EPSG:3857',
			      url: wfsServers[i].url,
			      typeName: typeName,
			      bbox: bbox
			    });	
    		}
      ...
      process.execute({
        ...
        success: function(output, body, responseText) {
          ...
          if ($.isArray(output.result)) {
            ...
            html += '<button id="btn-addLayer" type="button" class="btn btn-default">';
            html += '<i class="fa fa-bars fa-fw"></i>Add Layer</button>';
...
//              html += '<div class="form-tips"> Select the text above and copy to the clipboard.</div>';
//              
//              html += '<div><select id="download-data-type" style="height:27px">';
//              html += '    <option value="gml">GML</option>';
//              html += '    <option value="geojson">GeoJson</option>';
//              html += '    <option value="shape" selected="selected">Esri Shape</option>';
//              html += '</select>&nbsp;&nbsp;<button type="button" id="btn-download-data">download!</button>';
//              html += '&nbsp;&nbsp;<button type="button" id="btn-add-Map">지도에 추가</button>';
//              html += '</div>';
//             
//              $("#dialog-form").html(html);
...
//              
//              $("#btn-download-data").on("click", function(e) {
//            	  console.log(e);
//            	  var dataType = $("#download-data-type option:selected").val();
//              	 });
            });
            
            $('#btn-addLayer').click(function() {
            		ui.outputMap.vector.set('title', 'result Map');
            		repMap.addLayer(ui.outputMap.vector);
               });
            ui.activateTab('tab-results');
            if (!ui.outputMap) {
              ui.outputMap = {};
            ...
            if (extent[0] === extent[2]) {
              ...
              view.fitExtent(
                ui.outputMap.source.getExtent(), 
            	wps.mapSettings.extent,
                ui.outputMap.map.getSize());
            }
          } else {
...
var clickDownload = function (resText) {
	console.log(resText);
}
...
wps.ui.prototype.createLinkPaths = function() {
  ...
  links.attr("d",function(d) {
    ...
    if (d.x1 != undefined) {
    	if (isNaN(d.x1) || isNaN(d.x2) || isNaN(d.y1) || isNaN(d.y2))
    		return null;
    }
...
wps.ui.prototype.redraw = function() {
  ...
  nodeEnter.each(function(d,i) {
    if (d.label) {
    	if (mandatoryOption == "true" && d.type == "input" && d.required == false) {
    		return true;
    	}
...
wps.ui.nodeMousedbclick = function(ui, d) {
	if (d.type == "process") {
		wps.ui.getDescribeWindow(d);
...
wps.ui.prototype.createProcessRect = function(node) {
  ...
  var mainRect = node.append("rect").
    ...
    on("mousedown", $.proxy(wps.ui.nodeMouseDown, null, this)).
  	on("dblclick", $.proxy(wps.ui.nodeMousedbclick, null, this));
};
...
// wps describe view
wps.ui.getDescribeWindow = function (d) {
	while ($("#process-inf").length > 0) {
		$("#process-inf").remove();
	};
	var dataForm = 
   "<div id=\"process-inf\" style=\"font-size:11px;\">" +
   "<form id=\"process-inf-form\" class=\"form-horizontal\">" +
   "<p><b> Summary  note : </b>" +
   "<br /> " + d._info._abstract.value +	
   "<p><b> Data inputs : </b>" +
   "	<div id=\"process-inf-inputTable\" />" +
   "<br /> " + 
   "<p><b> Data output : </b> " +
   "	<div id=\"process-inf-outputTable\" />" +
   "</form>" +
   "</div>";
	...
	var inputTableTag = 
		'<table id="boottable-inputData" data-id-field="identifier" style="background-color:#ffffff;">' +
	    '<thead>' +
	    '	<tr>' +
		...
		'        <th data-field="surportedFormat" width="250">SurportedFormat</th>' +
		'        <th data-field="hidden" data-visible="false">hidden</th>'
	    '	</tr>' +
		'</thead>' +
	    '</table>';
	
	$('#process-inf-inputTable').append(inputTableTag);
	
	var inputJsonData = [];
	var inputDatas = d._info.dataInputs.input;
	for (var data in inputDatas) {
		inputJsonData.push({
			'Identifier' : inputDatas[data].identifier.value,
			'Title': inputDatas[data].title.value,
			'Type': inputDatas[data].complexData == undefined ? 'Literal' : 'Complex',
			'maxOccurs' : inputDatas[data].maxOccurs,
			'minOccurs' : inputDatas[data].minOccurs,
			'surportedFormat' : getsurportedFormatSelect(inputDatas[data]),
			'Hidden' : inputDatas[data]
		});
	};
	
 $('#boottable-inputData').bootstrapTable({
	    cache: false,
	    striped: false,
	    data: inputJsonData
 });
 ...
 var outputTableTag = 
		'<table id="boottable-outputData" data-id-field="identifier" style="background-color:#ffffff;">' +
	    '<thead>' +
	    '	<tr>' +
		'        <th data-field="Identifier">Identifier</th>' +
		'        <th data-field="Title">Title</th>' +
	...
	$('#process-inf-outputTable').append(outputTableTag);
	
	var outputJsonData =[];
	var outputDatas = d._info.processOutputs.output;
	for (var data in outputDatas) {
		outputJsonData.push({
			'Identifier' : outputDatas[data].identifier.value,
			'Title': outputDatas[data].title.value,
			'Type': outputDatas[data].complexOutput == undefined ? 'Literal' : 'Complex',
			'surportedFormat' : getsurportedFormatSelect(outputDatas[data]),
			'Hidden' : outputDatas[data]
		});
	};
	
	 $('#boottable-outputData').bootstrapTable({
		    cache: false,
		    striped: false,
		    data: outputJsonData
	 });
	 
	 function getsurportedFormatSelect(data) {
	 	var returnFormat = '';
	 	if (data.complexOutput != undefined) {
	 		var supportedFormatData = data.complexOutput.supported.format;
	 		returnFormat =  '<select class="form-control"> ';
	 		for (var format in supportedFormatData) {
	 			returnFormat += '<option>' + supportedFormatData[format].mimeType + '</option>';
	 		}
	 		returnFormat += '</select>';
	 	} else if (data.literalOutput != undefined) {
	 		returnFormat = data.literalOutput.dataType.value;
	 	} else if (data.complexData != undefined) {
	 		var supportedFormatData = data.complexData.supported.format;
	 		returnFormat =  '<select class="form-control"> ';
	 		for (var format in supportedFormatData) {
	 			returnFormat += '<option>' + supportedFormatData[format].mimeType + '</option>';
	 		}
	 		returnFormat += '</select>';
	 	} else if (data.literalData != undefined) {
	 		if (data.literalData.anyValue == undefined) {
	 			returnFormat = 'Text';
	 		} else {
	 			returnFormat = '-';
	 		}
	...
	$("#process-inf").dialog("option", "title", d.label).dialog( "open" );
};